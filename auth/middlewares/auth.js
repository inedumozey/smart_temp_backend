...
module.exports ={
    ...
    userAuthProfile: async(req, res, next)=>{
        try{
            const authToken = req.headers["authorization"];
           
            if(!authToken){
                return res.status(402).json({status: false, msg: "You are not authorize, please login or register1"})
            }
            // Verify token
            const token = authToken.split(" ")[1]
            ...
            const data = await jwt.verify(token, process.env.JWT_ACCESS_SECRET)
            if(!data){
                return res.status(402).json({status: false, msg: "You are not authorize, please login or register2"})
            }
            ...
            if(!user){
                return res.status(402).json({status: false, msg: "You are not authorize, please login or register3"})
            }
            ...
            if(err.message == 'invalid signature' || err.message == 'invalid token' || err.message === 'jwt malformed' || err.message === "jwt expired"){
                return res.status(402).json({status: false, msg: "You are not authorized! Please login or register4"})
            }
            return res.status(500).json({ status: false, msg: "Server error, please contact customer support"})
        }
    },
